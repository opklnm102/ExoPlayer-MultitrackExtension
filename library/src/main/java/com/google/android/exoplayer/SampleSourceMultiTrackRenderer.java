 ...
 * {@link SampleSource}.
 */
public abstract class SampleSourceMultiTrackRenderer extends TrackRenderer {

  public static final String TAG = SampleSourceMultiTrackRenderer.class.getSimpleName();
   ...
   */
  public SampleSourceMultiTrackRenderer(SampleSource... sources) {
  ...
  // doPrepare()를 override하기 위해 final 해제
  @Override
  ...
  protected void onEnabled(int track, long positionUs, boolean joining)
    ...
    enabledSource = sources[handledSourceIndices[track]];  // 1개밖에 없다
    enabledSourceTrackIndex = handledSourceTrackIndices[track];

    Log.d(TAG, "track " + track);  //0
    Log.d(TAG, "handledSourceTrackIndices track " + handledSourceTrackIndices[track]);  // 1

    // 사용 가능한 audio track가 n개에 대해 enabledSource enable
    for(int i=0; i<getTrackCount(); i++){
      enabledSource.enable(enabledSourceTrackIndex + i, positionUs);
    }
  ...
  protected final int readSource(long positionUs, MediaFormatHolder formatHolder,
      SampleHolder sampleHolder, int trackIndex) {
    // track을 바꿔준다.
    enabledSourceTrackIndex = handledSourceTrackIndices[trackIndex];  // handledSourceTrackIndices -> 1 or 2...
